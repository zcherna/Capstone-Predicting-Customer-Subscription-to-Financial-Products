...
The first step in my modeling process was to perform data preprocessing. I created a new column called ‘made_deposit_binary’, which assigned a 0 to customers who did not subscribe and a 1 to customers who did subscribe to a term deposit. I then created a numeric pipeline to StandardScale numeric data, and a categorical pipeline to OneHotEncode categorical data. I then assigned variables to the numeric and categorical feature columns, and instantiated a column transformer containing the mini pipelines. My next step was to assign variables to my predictors and target, and then split the data into a training and test set. To easily obtain cross validation scores and visualize my models, I imported the "ModelWithCV()" class provided by Flatiron which uses 10 as the kfolds parameter. As I mentioned earlier, there is a class imbalance in this dataset. To account for this, I used the SMOTE oversampling technique, however the results did not show any accuracy improvement over the models without SMOTE. I then utilized GridSearch in order to perform hyperparameter tuning. The results from all the preliminary models can be found in the [Jupyter Notebook](./Final_Notebook/Jupyter_Notebook.ipynb). Finally, I  looked at other metrics to further determine the efficacy of the model. ROC_AUC is a measurement of the ratio of the rate of True Positive to False postive, with a higher score being better. Precision measures the proportion of positive identifications that is correct. Recall measures the proportion of actual positives correctly identified. F1 score is a metric which combines precision and recall. In some cases, it may be better to build a model that optimizes for a reduction in false positives over accuracy. In this case, my assumption is the cost of a false positive is not prohibitively expensive. I am willing to accept false positives if it means my accuracy will be greater. 
In the next section, I discuss the results of my final models. 
# Final Evaluation
The best model turned out to be the Gradient Boost Classifier with the following hyperparameters, discovered through Grid Search:
